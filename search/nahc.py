...
class NAHC:
    ...
    def execute_search(self, iterations, max_iterations):
        ...
        for i in range(iterations):
            neighbours = self.__generate_neighbours(solution) #se genereaza vecinii unei solutii

            found = False
            for neighbour in neighbours:
                number_of_neighbours += 1
                neighbour_quality = self.__evaluate_solution(neighbour) #fiecare vecin se evalueaza si se verifica daca este mai bun decat solutia curenta

                if neighbour_quality > solution_quality:
                    solution = neighbour
                    solution_quality = neighbour_quality
                    break                                     #daca gasim un vecin mai bun decat solutia se opreste cautarea

                if number_of_neighbours % max_iterations == 0 and found is False: #daca nu gasim ceva mai bun dupa numarul maxim de iteratii, se reinitializeaza solutia si se evalueaza din nou
                    solution = self.__generate_solution()
                    solution_quality = self.__evaluate_solution(solution)
        ...
        return self.__best_quality #la final se returneaza cea mai buna solutie
    ...
    def __generate_neighbours(solution):
        ...
        for i in range(len(solution)):
            ...
            solution_copy[i] = 1 - solution_copy[i] #se inverseaza valoarea solutiei copiate de la pozitia "i"
